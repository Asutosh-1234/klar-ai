...
export function AetherAgentsView() {
  ...
  const totalAutomatedActions = agents.reduce((acc, a) => acc + a.tasksCompleted, 0);
  const avgAccuracy = agents.length > 0
    ? (agents.reduce((acc, a) => acc + parseFloat(a.accuracy), 0) / agents.length).toFixed(1) + '%'
    : '0.0%';
      ...
      <div className="p-8 max-w-5xl mx-auto flex flex-col gap-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <div className="bg-surface-container/60 border border-white/5 rounded-xl p-5 flex flex-col justify-between h-28">
            ...
            <span className="text-2xl font-bold text-white">{activeAgentsCount} <span className="text-xs font-normal text-primary">online</span></span>
          </div>
          <div className="bg-surface-container/60 border border-white/5 rounded-xl p-5 flex flex-col justify-between h-28">
            <span className="text-[10px] font-bold text-on-surface-variant opacity-60 uppercase">Automated Actions</span>
            <span className="text-2xl font-bold text-white">{totalAutomatedActions} <span className="text-xs font-normal text-on-surface-variant">this week</span></span>
          </div>
          <div className="bg-surface-container/60 border border-white/5 rounded-xl p-5 flex flex-col justify-between h-28">
            <span className="text-[10px] font-bold text-on-surface-variant opacity-60 uppercase">Avg. Accuracy</span>
            <span className="text-2xl font-bold text-white">{avgAccuracy} <span className="text-xs font-normal text-emerald-400">verified</span></span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {agents.length === 0 ? (
            <div className="col-span-2 bg-surface-container/60 border border-white/5 rounded-xl p-8 text-center text-on-surface-variant opacity-60 text-xs">
              No agents configured or running
            </div>
          ) : (
            agents.map((agent) => (
